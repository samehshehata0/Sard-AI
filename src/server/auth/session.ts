...
export async function createSession(userId: string): Promise<void> {
  ...
  if (existingToken) await AuthSessionModel.deleteOne({ tokenHash: hashSessionToken(existingToken) });

  const token = randomBytes(32).toString("base64url");
  const expiresAt = new Date(Date.now() + SESSION_DURATION_SECONDS * 1_000);
  await AuthSessionModel.create({ userId, tokenHash: hashSessionToken(token), expiresAt });
...
export async function requireRole(...allowedRoles: UserRole[]): Promise<AuthUserDto> {
...
export async function clearSession(): Promise<void> {
  const cookieStore = await cookies();
  const token = cookieStore.get(SESSION_COOKIE_NAME)?.value;
  cookieStore.set(SESSION_COOKIE_NAME, "", { ...cookieOptions(new Date(0)), maxAge: 0 });
  if (token) {
    await connectToDatabase();
